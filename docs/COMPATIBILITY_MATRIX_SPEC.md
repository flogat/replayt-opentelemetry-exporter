...
**Implementation gap (until Builder ships backlog *Expand CI matrix to include Python 3.11 (requires-python parity)*):** The workflow may still run the four cells on **3.12** only and rely on job **`test-python-3-11`** for a **single** pin set on **`schedule` / `workflow_dispatch`**. That layout is **transitional**; README and **§2** MUST keep **declared vs tested** accurate until **`test`** gains the **3.11** dimension as above. When parity lands, Builder **SHOULD remove** **`test-python-3-11`** and any **`schedule` cron** that exists **only** for it (see **§4.3**).
...
### 4.3 Transitional supplemental job (`test-python-3-11`) — superseded by §4.1 parity

While **§4.1** parity is **not** yet implemented, the repository MAY retain job **`test-python-3-11`** as a **partial** check on **3.11** (one pin set, **`schedule` + `workflow_dispatch`**). That job **does not** satisfy **requires-python parity** by itself; it only bridges until **`test`** runs the **full** four cells on **3.11**.

When **§4.1** is satisfied, **remove** **`test-python-3-11`** (and cron-only triggers tied only to it) unless maintainers amend this spec and README with a **new** rationale for duplicate coverage (discouraged).
...
| **Spec (phase 2)** — *Expand CI matrix with optional Python 3.11 job* | Historical: merge-gate policy superseded by *Expand CI matrix to include Python 3.11 (requires-python parity)*; **§4.1** / **§4.3** retain transitional supplemental-job wording. |
| **Spec (phase 2)** — *Expand CI matrix to include Python 3.11 (requires-python parity)* | **§4.1** merge-blocking **3.11** + **3.12** × four replayt×OTel cells; **§4.2** item **4**; **§4.3** transitional job; [CI_SPEC.md](CI_SPEC.md) **§2.2**, **§3.6**, **§5** items **6–7**; [PUBLIC_API_SPEC.md](PUBLIC_API_SPEC.md) **§1.1** / **§8** item **14**; README **Version compatibility** (**declared vs tested**); CHANGELOG **Unreleased** notes this spec pass. |
...
| **Builder (phase 3+)** — *Expand CI matrix to include Python 3.11 (requires-python parity)* | Job **`test`** implements **§4.1** (eight matrix rows or equivalent); remove or justify **`test-python-3-11`** per **§4.3**; README **CI / validation** column; **`tests/test_ci_workflow.py`**; Ruff + pytest match **[CI_SPEC.md](CI_SPEC.md) §3.1** and README; CHANGELOG **Unreleased** when behavior ships. |
