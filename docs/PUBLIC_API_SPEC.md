...
The backlog item *Expand CI matrix with optional Python 3.11 job* is **superseded** for merge-gate expectations by *Expand CI matrix to include Python 3.11 (requires-python parity)* below. Historical supplemental-job behavior remains documented only as **transitional** in [COMPATIBILITY_MATRIX_SPEC.md](COMPATIBILITY_MATRIX_SPEC.md) **§4.3** until YAML catches up.
...
- CI job **`.github/workflows/ci.yml`** **`test`** runs the replayt×OpenTelemetry **four-cell** matrix (see [COMPATIBILITY_MATRIX_SPEC.md](COMPATIBILITY_MATRIX_SPEC.md) **§4.1**): replayt **0.4.0** and **latest**, OpenTelemetry API/SDK **1.20.0** and **1.40.0**, with resolved versions printed each cell. **Normative:** that matrix MUST run on **Python 3.11** and **3.12** on the merge gate (**requires-python parity**); **today** the workflow may still run those four cells on **3.12** only—see README **Version compatibility** implementation note.
- Job **`test-python-3-11`** (transitional): one pin set on **3.11**, **`schedule` / `workflow_dispatch`** only, until **`test`** includes **3.11**—[COMPATIBILITY_MATRIX_SPEC.md](COMPATIBILITY_MATRIX_SPEC.md) **§4.3**, [CI_SPEC.md](CI_SPEC.md) **§3.6**.
...
| --------- | ---------------- | ----- |
| Python | `requires-python` (currently `>=3.11`) | **Declared** floor **3.11**. **Normative merge gate:** full four-cell matrix on **3.11** and **3.12** ([COMPATIBILITY_MATRIX_SPEC.md](COMPATIBILITY_MATRIX_SPEC.md) **§4.1**). **Transitional:** four cells on **3.12** only + **`test-python-3-11`** on **3.11** ([COMPATIBILITY_MATRIX_SPEC.md](COMPATIBILITY_MATRIX_SPEC.md) **§4.3**) until Builder ships parity. |
...
14. **Python 3.11 requires-python parity** — [COMPATIBILITY_MATRIX_SPEC.md](COMPATIBILITY_MATRIX_SPEC.md) **§4.1** and [CI_SPEC.md](CI_SPEC.md) **§3.6**: merge gate runs the **full** replayt×OpenTelemetry matrix on **3.11** and **3.12**; Ruff + pytest match **§3.1** and README on every row; **`test-python-3-11`** removed or re-justified per **§4.3** when parity lands; **§7.2**–**§7.3** here and README **Version compatibility** stay aligned (**declared vs tested**).
