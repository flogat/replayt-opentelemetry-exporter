...
- **Current Builder outcome:** [COMPATIBILITY_MATRIX_SPEC.md](COMPATIBILITY_MATRIX_SPEC.md) **§7.2** records a **2026-03-29** PyPI audit: **no** published **2.x** for **`opentelemetry-api`** or **`opentelemetry-sdk`** (including pre-releases in the index at that check). Support for **2.x** stays **out of scope** until those packages exist and maintainers complete the full **§7.2** spike (**1–3**) plus **§7.3–7.4** for a *support* merge.
